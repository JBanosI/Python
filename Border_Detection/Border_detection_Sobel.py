...
for M2 in range(1, M-1):
    for N2 in range(1, N-1):
        border = int(-E_D_G[M2-1, N2-1] - E_D_G[M2-1, N2] - 2*E_D_G[M2-1, N2+1] + 2*E_D_G[M2+1, N2-1] + E_D_G[M2+1, N2] + E_D_G[M2+1, N2+1])
        if(border < 0):
            B_X[M2, N2] = 0
        else:
            B_X[M2, N2] = border
...
for M2 in range(1, M-1):
    for N2 in range(1, N-1):
        border = int(-E_D_G[M2-1, N2-1] + E_D_G[M2-1, N2+1] - 2*E_D_G[M2, N2-1] + 2*E_D_G[M2, N2+1] - E_D_G[M2+1, N2] + E_D_G[M2+1, N2+1])
        if(border < 0):
            B_Y[M2, N2] = 0
        else:
            B_Y[M2, N2] = border
...
cv2.imwrite("sobel_hor.jpg",B_X)
cv2.imshow('Bordes_Y', B_Y)
cv2.waitKey(0) #Espera una tecla
cv2.imwrite("sobel_ver.jpg",B_Y)
